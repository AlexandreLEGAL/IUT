...
candidatures = [("Kim Gordon", "Apito"), ("Superman", "Trompette"), ("Frank Zappa", "Banjo"),
                ("Chico Science", "Apito"), ("Frank Zappa", "Trompette")]
...
def premier_candidat(candidatures, instrument):
    ...
    ok = False
    i = 0
    while i < len(candidatures) and not ok:
        if candidatures[i][1] == instrument:
            res = candidatures[i][0]
            ok = True
        i += 1
    return res
...
assert premier_candidat(candidatures, 'Guitarron') == ""
...
def tous_candidats(candidatures):
    ...
    """
    res = set()
    for (candidat, _) in candidatures:
        if candidat not in res:
            res.add(candidat)
    return res


# TESTS A COMPLETER ET/OU CORRIGER
assert tous_candidats(candidatures) == {"Kim Gordon", "Superman", "Chico Science", "Frank Zappa"}
...
def compose_fanfare(candidatures):
    """
    param: candidatures est une liste de tuples (nom, instrun) où nom et instrum sont des str renvoie un dictionnaire
    dont les clés sont les personnes qui seront invitées à faire partie de la fanfare, et les valeurs sont les
    instruments dont ils joueront. Pour cela, nous prenons pour chaque instrument la personne qui a candidaté en
    premier, sauf si elle joue déjà d'un autre instrument dans la fanfare.
    """
    res = {}
...
# TESTS A COMPLETER ET/OU CORRIGER
assert compose_fanfare(candidatures)== {'Kim Gordon': {'Apito'}, 'Superman': {'Trompette'}, 'Frank Zappa': {'Trompette', 'Banjo'}, 'Chico Science': {'Apito'}}
...
exemple_candidatures = [('Albert', 'Triangle'), ('Fanny', 'Triangle'), ('Gérard', 'Clarinette'),
                        ('Albert', 'Clarinette'), ('Gérard', 'Hélicon'), ('Basile', 'Triangle'), ('Carole', 'Triangle'),
                        ('David', 'Triangle'), ('Henri', 'Hélicon'), ('Basile', 'Clarinette'),
                        ('Isaline', 'Clarinette')]
